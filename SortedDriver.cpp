...
mostIsolated(vector<double> & number)
{
	...
	double min_distance;
	double nearest_number;
	for (int i = 1; i < number.size(); i++)
	{
		front_distance = abs(number.at(i) - number.at(i - 1));
		if ((i + 1) < number.size())
		{
			back_distance = abs(number.at(i + 1) - number.at(i));
		}
		if (front_distance > back_distance)
		{
			min_distance = back_distance;
		}
...
unmatched(list<string> & A, list<string> & B)
{
	// STUB  STUB  STUB
	double matches = 0;
	double non_matches = 0;
	for (std::list<string>::iterator itA = A.begin(), itB = B.begin(); itA != A.end() && itB != B.end();)  //recieved help from Jagadish on this matter
	{
		if (*itA == *itB)
		{
			++itA;
